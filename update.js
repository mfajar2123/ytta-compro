...
const raw = `Jakarta Utara - Batavia PIK	Rukan Golf Island Blok C No.36, Pantai Indah Kapuk, Jakarta Utara	0822-9999-5125	09.00-17.00  	https://share.google/IpaWoV3Os0TJKfkiO
...
for (const line of lines) {
  ...
  // This handles basic tab separation
  const cols = line.split('\t');
  if (cols.length >= 5) {
    // Clean quotes and newlines
    const name = cols[0].replace(/^"|"$/g, '').replace(/\\n/g, ' ').trim();
    const address = cols[1].replace(/^"|"$/g, '').replace(/\\n/g, ' ').trim();
    const phone = cols[2].replace(/^"|"$/g, '').replace(/\\n/g, ' ').trim();
    const hours = cols[3].replace(/^"|"$/g, '').replace(/\\n/g, ' ').trim();
    const mapCoordinates = cols[4].replace(/^"|"$/g, '').replace(/\\n/g, ' ').trim();
...
const dataFile = path.join(__dirname, 'data/siteData.json');
const siteData = JSON.parse(fs.readFileSync(dataFile, 'utf8'));

siteData.stores = stores;

fs.writeFileSync(dataFile, JSON.stringify(siteData, null, 2), 'utf8');
console.log('Successfully updated stores array to length: ' + stores.length);
