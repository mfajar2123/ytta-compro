<template>
  <section class="py-24 md:py-32 bg-whitedust border-b border-gray-200">
      ...
      <div class="relative flex w-full overflow-hidden group">
        ...
        <div class="animate-marquee-infinite flex whitespace-nowrap items-center shrink-0">
          <div v-for="i in 8" :key="`p1-${i}`" class="px-8 md:px-12 lg:px-16 flex items-center justify-center">
            <NuxtImg :src="siteData.partners[(i - 1) % siteData.partners.length].logo" :alt="siteData.partners[(i - 1) % siteData.partners.length].name" class="w-32 h-16 sm:w-40 sm:h-20 md:w-48 md:h-24 lg:w-64 lg:h-32 object-contain hover:scale-110 transition-all duration-500 drop-shadow-sm cursor-pointer" loading="lazy" format="webp" fit="inside" width="256" height="128" />
          </div>
        </div>
