...
const rawStores = `Jakarta Utara - Batavia PIK	Rukan Golf Island Blok C No.36, Pantai Indah Kapuk, Jakarta Utara	0822-9999-5125	09.00-17.00  	https://share.google/IpaWoV3Os0TJKfkiO
...
// A simple manual parser to handle the specific TSV format with newlines in quotes
const stores = [];
let idx = 0;

// Split by tab, but handle quotes
function parseTSV(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (char === '"') {
      ...
      if (entries.length >= 5) {
        stores.push({
          id: 'st-' + (stores.length + 1),
          name: entries[0].replace(/^"|"$/g, '').trim(),
          address: entries[1].replace(/^"|"$/g, '').trim().replace(/\\n/g, ' '),
          phone: entries[2].replace(/^"|"$/g, '').trim().replace(/\\n/g, ' '),
          hours: entries[3].replace(/^"|"$/g, '').trim().replace(/\\n/g, ' '),
          note: "BUKA SETIAP HARI",
          mapCoordinates: entries[4].replace(/^"|"$/g, '').trim()
        });
      }
      entries = [];
      current = '';
...
const siteDataPath = path.join(__dirname, 'data', 'siteData.json');
const siteData = JSON.parse(fs.readFileSync(siteDataPath, 'utf8'));

siteData.stores = stores;

fs.writeFileSync(siteDataPath, JSON.stringify(siteData, null, 2), 'utf8');
console.log('Successfully written ' + stores.length + ' stores.');
